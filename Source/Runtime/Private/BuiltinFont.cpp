...
namespace won::builtinfont
{
    namespace
    {
        ...
        // Modern DOS 8x16 CP437 bitmap font (susam/pcface, CC0). 256 glyphs, 16 rows each;
        // each row byte is 8 horizontal pixels, MSB (bit 7) = leftmost.
        constexpr uint8_t font_8x16[256][16] =
        {
    ...
    bool BuildAtlas(rendering::RHIDevice& device)
    {
        ...
        Vector<uint8> pixels(static_cast<Size>(atlas_width) * static_cast<Size>(atlas_height), 0);
        for (int glyph = 0; glyph < glyph_count; ++glyph)
        {
            const int cell_x = (glyph % atlas_cols) * glyph_width;
            const int cell_y = (glyph / atlas_cols) * glyph_height;
            for (int row = 0; row < glyph_height; ++row)
            {
                const uint8 row_bits = font_8x16[glyph][row];
                for (int col = 0; col < glyph_width; ++col)
                {
                    if ((row_bits >> (7 - col)) & 1)
                    {
                        pixels[static_cast<Size>(cell_y + row) * atlas_width + (cell_x + col)] = 0xff;
                    }
                }
            }
        }

        rendering::RHITextureDesc desc = {};
        ...
        desc.array_layers = 1;
        desc.sample_count = 1;
        desc.format = rendering::RHIFormat::R8Unorm;
        desc.usage = rendering::RHIResourceUsage::Default;
        desc.bind_flags = rendering::RHIBindFlags::ShaderResource;

        atlas_texture = device.CreateTexture(desc, pixels.data(), pixels.size());
        if (!atlas_texture)
        {
        ...
        if (!device.CreateSubresource(*atlas_texture, srv_desc, &atlas_srv))
        {
            backlog::Post("[BuiltinFont] atlas SRV creation failed", backlog::LogLevel::Error);
            atlas_texture.reset();
            return false;
        }
        return true;
