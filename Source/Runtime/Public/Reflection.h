...
namespace won::reflection
{
    ...
    // We use runtime registry for supporting custom components

    WONENGINE_API bool RegisterType(const won::TypeDesc* type_desc);
    WONENGINE_API bool UnregisterType(TypeId type_id);
    WONENGINE_API const won::TypeDesc* FindType(TypeId type_id);
...
// Below is the compile-time meta-generation code
// for built-in types
...
template<> struct TypeMeta<type> { \
    ...
    inline static constexpr won::TypeDesc desc = { sizeof(won::TypeDesc), #type, display_name_value, value_type_value, static_cast<uint32_t>(sizeof(ReflectedType)), static_cast<uint32_t>(alignof(ReflectedType)), nullptr, nullptr, nullptr, nullptr, 0, nullptr, 0 }; \
...
#define WON_REFLECT_FIELD(member, value_type_value, flags_value) \
        { sizeof(won::FieldDesc), #member, #member, value_type_value, "", static_cast<uint32_t>(offsetof(ReflectedType, member)), static_cast<uint32_t>(sizeof(std::declval<ReflectedType>().member)), flags_value },

#define WON_REFLECT_FIELD_TYPED(member, field_type, value_type_value, flags_value) \
        { sizeof(won::FieldDesc), #member, #member, value_type_value, #field_type, static_cast<uint32_t>(offsetof(ReflectedType, member)), static_cast<uint32_t>(sizeof(std::declval<ReflectedType>().member)), flags_value },

#define WON_REFLECT_STRUCT_END() \
        { sizeof(won::FieldDesc), "", "", ValueType::Unknown, "", 0, 0, FieldFlagNone } \
    }; \
    ...
    inline static constexpr won::TypeDesc desc = { sizeof(won::TypeDesc), type_name, display_name, reflected_value_type, static_cast<uint32_t>(sizeof(ReflectedType)), static_cast<uint32_t>(alignof(ReflectedType)), &Construct, &Destruct, &Copy, fields, static_cast<uint32_t>(sizeof(fields) / sizeof(fields[0]) - 1), nullptr, 0 }; \
    static const won::TypeDesc* Get() { return &desc; } \
};
...
#define WON_REFLECT_ENUM_END() \
    ...
    inline static constexpr won::TypeDesc desc = { sizeof(won::TypeDesc), type_name, display_name, ValueType::Enum, static_cast<uint32_t>(sizeof(ReflectedType)), static_cast<uint32_t>(alignof(ReflectedType)), nullptr, nullptr, nullptr, nullptr, 0, enum_values, static_cast<uint32_t>(sizeof(enum_values) / sizeof(enum_values[0])) }; \
    static const won::TypeDesc* Get() { return &desc; } \
};
