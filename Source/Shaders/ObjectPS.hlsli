...
float4 main(PixelInput input, in bool is_frontface : SV_IsFrontFace) : SV_Target
{
    ...
    if (material.textures[NORMALMAP].IsValid())
    {
        ...
        half3 normal = DecodeTangentNormal(material.textures[NORMALMAP].Sample(sampler_objectshader, uvsets).xy);
...
#ifndef WON_SHIPPING
    final_color = ApplyDebugViewMode(final_color, surface, base_color, metallic, input.pos.xy, GetGeometry().lod_index);
#endif
