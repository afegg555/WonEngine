...
PixelInput main(VertexInput input)
{
    ...
    const uint sprite_flags = push.GetFlags();
    ...
    if ((sprite_flags & SHADER_SPRITE_FLAG_PARTICLE) != 0) // this is ok, in cpu side we will seperate draw calls for particle and non-particle sprite
    {
        ...
        StructuredBuffer<float4> particle_buffer = bindless_buffers_float4[DescriptorIndex(push.GetResourceIndex())];
        const float3 center = particle_buffer[push.instance_index * 2u].xyz;
        color = (half4)particle_buffer[push.instance_index * 2u + 1u];
